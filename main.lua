--[[
	...
	--// Floating Button (replaces the old yellow Minimize button)

	local MobileButton = Window:CreateMobileMinimizer({
		Image = "rbxassetid://0",
...
function Library:CreateWindow(Settings: { Name: string, Title: string, Subtitle: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
	...
	local Top = Sidebar:FindFirstChild("Top");
	...
	--// "Name" is the new field; fall back to "Title" for backwards compatibility
	Settings.Title = Settings.Title or Settings.Name

	if Top then
		local TitleLabel = Top:FindFirstChild("Title")
		local SubtitleLabel = Top:FindFirstChild("Subtitle")

		if TitleLabel then SetProperty(TitleLabel, { Text = Settings.Title }) end
		if SubtitleLabel and Settings.Subtitle then SetProperty(SubtitleLabel, { Text = Settings.Subtitle }) end
	end
	...
	for Index, Button in next, Sidebar.Top.Buttons:GetChildren() do
		if Button:IsA("TextButton") then
			...
			if Name == "Minimize" then
				--// Yellow button removed: minimizing is now handled by the floating Mobile Minimizer button
				Destroy(Button)
				continue
			end
	...
	--// Floating Mobile Minimizer (replacement for the old yellow button)

	function Options:CreateMobileMinimizer(Settings: { Image: string, BackgroundColor3: Color3, Shape: string })
		...
		Corner.CornerRadius = (Settings.Shape == "Circle") and UDim.new(1, 0) or UDim.new(0, 14);
